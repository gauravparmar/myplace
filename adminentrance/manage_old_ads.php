...
<table width="98%" cellspacing="10">  
    ...
    <tr >
    	<td>
        	<form  action="manage_old_ads.php?page_start=<?php 
				...
				<?php
					...
					{	
						...
						/**seaching algo start**/	
								...
								$query="SELECT * FROM `ads` WHERE $where_ad_title AND $where_ad_content AND $where_date_and_time ORDER BY 'TIMESTAMP' LIMIT $page_start, $page_end";
								$query_run=mysql_query($query);	
						/**seaching algo end**/			
						
						$no_of_ads_for_current_page=mysql_num_rows($query_run);
						/**--------------**/
						$query_for_all_records="SELECT `AD_ID` FROM `ads` WHERE $where_ad_title AND $where_ad_content AND $where_date_and_time";
						...
						else
						{
							...
							echo'	<tr>
										<td align="center">
											<font color="#666666" face="Trebuchet MS, Arial, Helvetica, sans-serif" style="font-weight:bold">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Showing '.($page_start+$no_of_ads_for_current_page).' out of '.$total_records_found.' ad',$suffix,' found. </font>'; 
							if($page_start>0)
								...
								echo '<tr >
									<td  class="ad_text_box" style="margin-top:15 0 0 0">
										<table cellpadding="1" cellspacing="0" border="0" width="100%" class="ad_text">
											<tr valign="top">
												<td colspan="2">
												<a href="manage_old_ads.php?id='.$ad_id.'" style="text-decoration:none;" class="ad_titles">'.$ad_title.'</a>&nbsp;<strong>On </strong>'.$date_and_time.'
												</td>
												
												<td colspan=2 align="right" ><a href="manage_old_ads.php?id='.$ad_id.'" class="full_details" font-size:15px;"><font class="full_details">Full details..</font></a>
												</td>
												<td width="50px">
													<input type="button" value="Edit" class="edit_button" onCLick="window.location=\'old_ad_edit.php?id='.$ad_id.'\'">
												</td>
												<td  width="50px">
													<input type="button" value="Delete" class="delete_button" onCLick="window.location=\'old_ad_delete.php?id='.$ad_id.'\'">
												</td>
											</tr>
										</table>
									</td>
							...
							echo '</table>';
							echo'<font color="#666666" face="Trebuchet MS, Arial, Helvetica, sans-serif" style="font-weight:bold">Showing '.($page_start+$no_of_ads_for_current_page).' out of '.$total_records_found.' ad',$suffix,' found. </font>';  
							if($page_start>0)
								echo'<input type="submit" value="Previous" name="previous" align="right" class="next_and_previous_button"/>';
							if(($page_start+$no_of_ads_to_show)<$total_records_found)
