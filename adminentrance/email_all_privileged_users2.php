                                    ...
                                    if(isset($_POST['subject'])||isset($_POST['message'])||isset($_POST['number']))
                                    {
                                        ...
                                        if(!empty($_POST['subject'])&&!empty($_POST['message'])&&!empty($_POST['number']))
                                        {
                                            ...
                                            if($_POST['number']==$generated_captcha)
                                            {
												...
												if($num_of_users>0)
												{
													$error_in_sending_email=true;
													while($user=mysql_fetch_assoc($query_run))
													{
														$to=$user['EMAIL_ID'];
														if(@mail($to,$subject,$message,$header))
														{
															$error_in_sending_email=false;
														}
														else
															$error_in_sending_email=true;
													}
													if($error_in_sending_email==true)
														echo ' <font class="suggestion_ok_notice">&nbsp;&nbsp;  Problem in sending email to some users.&nbsp;&nbsp;  </font>';
													if($error_in_sending_email==false)
														echo ' <font class="suggestion_ok_notice">&nbsp;&nbsp;  Email has been sent.&nbsp;&nbsp;  </font>';								
												}
												else
                                                ...
                                                echo '&nbsp;&nbsp;  The number you entered did not match.&nbsp;&nbsp;  ';
